                ...
                <div class="col-lg-3 col-md-4 col-sm-6">
                    ...
                    <a href="{{ $tramiteUrl }}" class="service-item d-block text-decoration-none text-reset" @if($tramite->mode === 'link' && $tramite->redirect_url) target="_blank" rel="noopener noreferrer" @endif>
                        ...
                        @if($tramite->logo_image)
                            <img src="{{ asset($tramite->logo_image) }}" alt="{{ $tramite->title_full }}" style="width:100px; height:50px; object-fit:contain; display:block; margin:0 auto 8px;">
                        ...
                        <h4 class="text-center">{{ $tramite->title_full }}</h4>
                        <p class="text-center">{{ Str::limit($tramite->title_short, 100) }}</p>
                    </a>
                </div>
            @empty
