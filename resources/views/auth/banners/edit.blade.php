            ...
            <div class="col-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('banners.update', $banner) }}" class="forms-sample" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                                            ...
                                            <div class="row g-3">
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" name="title" class="form-control" id="title" placeholder="Título" value="{{ old('title', $banner->title) }}" required>
                                                    </div>
                                                </div>
                                                <div class="col-md-3">
                                                    <div class="form-group">
                                                        <label for="button_text">Texto del botón</label>
                                                        <input type="text" name="button_text" id="button_text" class="form-control" placeholder="Ver más" value="{{ old('button_text', $banner->button_text) }}">
                                                    </div>
                                                </div>
                                                <div class="col-md-3">
                                                    <div class="form-group">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="url" name="button_url" id="button_url" class="form-control" placeholder="https://" value="{{ old('button_url', $banner->button_url) }}">
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                    <div class="form-group form-check form-switch">
                                                        <input type="hidden" name="is_published" value="0">
                                                        <input class="form-check-input" type="checkbox" role="switch" id="is_published" name="is_published" value="1" @checked(old('is_published', $banner->is_published))>
                                    ...
                                    <div id="bannerDescriptions" class="accordion-collapse collapse" aria-labelledby="bannerDescriptionsHeading" data-bs-parent="#bannerAccordion">
                                        <div class="accordion-body">
                                            <div class="form-group">
                                                ...
                                                <textarea name="short_description" id="short_description" class="form-control" maxlength="180" rows="2" placeholder="Ingresa una descripción corta">{{ old('short_description', $banner->short_description) }}</textarea>
                                            </div>
                                            <div class="form-group">
                                                <label for="long_description">Descripción larga</label>
                                                <textarea id="long_description" class="form-control" name="long_description" rows="6">{{ old('long_description', $banner->long_description) }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <div class="accordion-body">
                                            <div class="row g-3">
                                                ...
                                                <div class="col-md-4">
                                                    <div class="form-group">
                                                        ...
                                                        <select class="form-control" id="media_type" name="media_type" required>
                                                            <option value="image" @selected(old('media_type', $banner->media_type)==='image')>Imagen</option>
                                                            <option value="video" @selected(old('media_type', $banner->media_type)==='video')>Video</option>
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="col-md-8">
                                                ...
                                                <div class="col-12" id="upload_wrapper">
                                                    <div class="form-group">
                                                        <label for="media">Archivo (imagen o video)</label>
                                                ...
                                                <div class="col-12">
                                                        <div class="mb-2">
                                                            @php
                                                                $rawPath = $banner->media_path ?? '';
                                                                $isExternal = \Illuminate\Support\Str::startsWith($rawPath, ['http://','https://']);
                                                                $mediaSrc = $isExternal ? $rawPath : ($rawPath ? asset(ltrim($rawPath, '/')) : '');
                                                            @endphp
                                                            @if($banner->media_type==='video')
                                                                @if(!$isExternal && $mediaSrc)
                                                                    <video src="{{ $mediaSrc }}" style="max-width: 320px;" controls muted></video>
                                                                @else
                                                                    <div class="video-icon" style="font-size:28px;color:#6c757d;"><i class="fas fa-video"></i></div>
                                                                @endif
                                                            @else
                                                                @if($mediaSrc)
                                                                    <img src="{{ $mediaSrc }}" alt="image" style="max-width: 320px; height: auto; object-fit: cover;" />
                                                                @endif
                                                            @endif
                                                        </div>
                                                </div>
                                                ...
                                                <div class="col-12">
                                                    <input type="hidden" name="position_x" id="position_x" value="{{ old('position_x', $banner->position_x ?? 50) }}">
                                                    <input type="hidden" name="position_y" id="position_y" value="{{ old('position_y', $banner->position_y ?? 50) }}">
                                                    <div class="slider-preview-wrapper" id="slider_preview_wrapper">
                                                        <div class="slider-preview" id="slider_preview" data-initial-image="{{ $banner->media_type==='image' ? asset(ltrim($banner->media_path, '/')) : '' }}" data-initial-x="{{ $banner->position_x ?? 50 }}" data-initial-y="{{ $banner->position_y ?? 50 }}">
    ...
    <script>
        $(function () {
            ...
            function setPreviewVideo(url){
                // if youtube, embed iframe to allow autoplay/mute/loop via embed params
                ...
                $preview.html('<video src="'+url+'" class="w-100 h-100" style="object-fit: cover; height:100%;" autoplay muted loop playsinline></video>');
                $preview.css({'background-image':'none'}).addClass('has-image').removeClass('no-helper');
                showPreview();
            }
            ...
            function applyUrlPreview(url){ var type = $('#media_type').val(); if(!url) { clearPreview(); return; }
                if(type==='video'){
                    // embed YouTube or play direct video URL
                    var youtubeMatch = url.match(/(?:youtube\.com\/(?:watch\?v=|embed\/)|youtu\.be\/)([A-Za-z0-9_\-]{11})/);
                    if(youtubeMatch){ setPreviewVideo(url); return; }
                    setPreviewVideo(url);
                } else {
            ...
            if(initialImage){ setPreviewImage(initialImage, $preview.data('initial-x'), $preview.data('initial-y')); }

            // initialize preview for existing video (local or external)
            var bannerMedia = @json($banner->media_path ?? '');
            var bannerType = @json($banner->media_type ?? '');
            if(bannerType === 'video' && bannerMedia){
                // if external URL (http(s)) use it directly, otherwise build asset path
                var isExternal = /^(https?:)?\/\//i.test(bannerMedia);
                if(isExternal){ applyUrlPreview(bannerMedia); }
                else { var mediaLocal = '{{ $banner->media_path ? asset(ltrim($banner->media_path, '/')) : '' }}'; if(mediaLocal) setPreviewVideo(mediaLocal); }
            }
